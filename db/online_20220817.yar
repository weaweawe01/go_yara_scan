...
strings:
	...
	$execu2 = "V4ZWN1"
	$execu3 = "ZXhlY3"
	$esystem4 = /lc3lzdGVt|VzeXN0ZW|ZXN5c3Rlb/ ascii
	$opening4 = /vcGVuaW5n|vcGVuaW5n|b3BlbmluZ/ ascii
	$fp1 = { D0 CF 11 E0 A1 B1 1A E1 }
	$fp6 = /BEGIN CERTIFICATE|POST \/|GET \/|YXBpLnRlbGVncmFtLm9/ ascii
condition:
	not any of ( $fp* ) and any of ( $decode* ) and ( ( any of ( $one* ) and not any of ( $execu* ) ) or any of ( $two* ) or any of ( $three* ) or ( any of ( $four* ) and not any of ( $esystem* ) ) or ( any of ( $five* ) and not any of ( $opening* ) ) or any of ( $six* ) or any of ( $seven* ) or any of ( $eight* ) or any of ( $nine* ) )
}
...
strings:
	...
	$gen_bit_sus73 = /(\.htpasswd|hacking|Hacking|"\?>"\.|\$cmd|\$password="|\$password='|whoami|portscan|Cyber|\/bin\/sh|"execute"|'cmd'|dumper|authorized_keys| \^ \$|suhosin|bypass|Shell|shell_|<PRE>|<pre>|crack|Content-Transfer-Encoding: Binary)/ ascii
...
rule Php_Webshell_webshell_php_generic_backticks_obfuscated
{
strings:
	$s1 = /echo[\t ]*\(?`\$/ ascii
	$php_short = "<?" wide ascii
...
rule Php_Webshell_webshell_php_includer_eval
{
strings:
...
rule Php_Webshell_webshell_php_includer_tiny
{
strings:
	$php_include1 = /include\(\$_(GET|POST|REQUEST)\[/ nocase ascii
	$php_short = "<?" wide ascii
	$no_xml1 = "<?xml version" nocase wide ascii
	$no_xml2 = "<?xml-stylesheet" nocase wide ascii
	$no_asp1 = "<%@LANGUAGE" nocase wide ascii
...
rule Php_Webshell_webshell_php_unknown_1
{
strings:
	$sp0 = /^<\?php \$[a-z]{3,30} = '/ ascii
...
rule Php_Webshell_webshell_php_double_eval_tiny
{
strings:
	$payload = /(\beval[\t ]*\([^)]|\bassert[\t ]*\([^)])/ nocase ascii
...
rule Multios_Trojan_merlin_agent_01
{
strings:
	$s1 = "merlin-agent/commands" ascii
	$s2 = "merlin-agent/cli/cli.go" ascii
	$s3 = "merlin-agent/agent.(*Agent)." ascii
condition:
	uint16(0) == 0x457f and 2 of them
}
rule Multios_Trojan_merlin_agent_02
{
strings:
	$s1 = "\x09Hostname: %s"
	$s2 = "\x09Platform: %s"
	$s3 = "\x09User GUID: %s"
	$s4 = "Command output:\x0D\x0A\x0D\x0A%s"
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_ja3transport_tools_01
{
strings:
	$s1 = "/ja3transport.Browser" ascii
	$s2 = "/ja3transport.JA3Client" ascii
	$s3 = "/ja3transport.NewTransportInsecure" ascii
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_geacon_01
{
strings:
	$g = "geacon/cmd/" ascii
	$s2 = "packet.PullCommand" ascii
	$s3 = "cmd/sysinfo.GeaconID" ascii
	$s4 = "sysinfo.GetMagicHead" ascii
condition:
	$g and 2 of ($s*)
}
rule Multios_Trojan_shellcode_loader
{
strings:
	$mprotect_read_jump = { B2 07 B9 00 10 00 00 89 E3 C1 EB 0C C1 E3 0C B0 7D CD 80 85 C0 78 ?? 5B 89 E1 99 B2  6A B0 03 CD 80 85 C0 78 ?? FF E1 }
condition:
	uint16(0) == 0x457f and $mprotect_read_jump and filesize < 500
}
rule Multios_Trojan_blue_team_busybox
{
strings:
	$b = "BusyBox" ascii
	$s1 = "Loaded Plugin Info:" ascii
	$s2 = "Num\tPid\tName\tArgs" ascii
	$s3 = "incorrect header check" ascii
	$s4 = "invalid code -- missing end-of-block" ascii
	$s5 = "not buildin cmd, please use with path!" ascii
condition:
	uint16(0) == 0x457f and $b and 3 of ($s*)
}
rule Multios_Trojan_reverse_proxy_ligolo
{
strings:
	$b = "ligolo/cmd/ligoloc" ascii
	$s2 = "ligolo server" ascii
	$s3 = "Could not start SOCKS5 proxy" ascii
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_gostrip_file
{
strings:
	$s1 = "stripd by go-strip" ascii
	$s2 = ".gosymtab" ascii
	$s3 = ".go.buildinfo" ascii
condition:
	all of them
}
rule Multios_Trojan_Stowaway_tools
{
strings:
	$s0 = "Stowaway/agent" fullword ascii
	$s1 = "libp2p/go-reuseport" fullword ascii
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_Proxy_HackerTools
{
strings:
	$s0 = "iptables -D INPUT -p tcp -m string --string %s --algo bm -m recent --set --name %s --rsource -j ACCEPT"
	$s1 = "iptables -A INPUT -p tcp -m string --string %s --algo bm -m recent --set --name %s --rsource -j ACCEPT"
	$s2 = "iptables -A INPUT -p tcp -m string --string %s --algo bm -m recent --name %s --remove -j ACCEPT"
	$s3 = "iptables -t nat -A PREROUTING -p tcp --dport %s --syn -m recent"
condition:
	uint16(0) == 0x457f and 2 of them
}
rule Multios_Trojan_pupy_rat1
{
strings:
...
rule Multios_Trojan_pupy_rat2
{
strings:
	$p = "_pupy.error"
	$s2 = "/var/tmp/XXXXXX"
	$s3 = "ld_preload_inject_dll"
	$s4 = "memfd_is_supported"
	$s5 = "/run/shm/XXXXXX"
	$s6 = "/dev/shm/XXXXXX"
	$s7 = "mexec:stderr"
condition:
...
rule Multios_Trojan_sliver
{
strings:
	$g2 = ".go.buildinfo" ascii
	$s1 = "(*Shell).String" ascii
	$s2 = "(*NetConnPivot).String" ascii
	$s3 = "(*SSHCommandReq).String" ascii
	$s4 = "(*RegistryWriteReq).String" ascii
	$s5 = "(*ScreenshotReq).String" ascii
	$s6 = "(*DNSSessionInit).String" ascii
	$s7 = "(*ExecuteAssemblyReq).String" ascii
	$s8 = "(*Download).String" ascii
	$s9 = "(*UploadReq).String" ascii
condition:
	all of ($g*) and 3 of ($s*)
}
rule Multios_Trojan_goShellcode
{
strings:
	$g2 = ".go.buildinfo" ascii
	$s1 = "github.com/brimstone/go-shellcode.Run" ascii
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_impost3r_sudo
{
strings:
	$s1 = "[sudo] password for %s:" ascii
	$s2 = "/tmp/.bashrc" ascii
	$s3 = "/tmp/.cache" ascii
	$s4 = "/tmp/.impost3r" ascii
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_impost3r_ssh_su
{
strings:
	$s1 = "/tmp/.sshsucache" ascii
	$s2 = "modify_result" ascii
	$s3 = "sendSingleResult" ascii
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_chashell
{
strings:
	$s1 = "transport.sendDNSQuery" ascii
	$s2 = "transport.pollRead" ascii
	$s3 = "transport.DNSStream" ascii
	$s4 = "splitting.Splits" ascii
	$s5 = "(*Message).GetClientguid" ascii
	$g2 = ".go.buildinfo" ascii
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_hershell
{
strings:
	$s1 = "meterpreter.Meterpreter" ascii
	$s2 = "meterpreter.GetRandomString" ascii
	$s3 = "shell.GetShell" ascii
	$s4 = "shell.InjectShellcode" ascii
	$g2 = ".go.buildinfo" ascii
condition:
	uint16(0) == 0x457f and all of them
}
rule Multios_Trojan_kubesploit
{
strings:
...
rule Multios_Trojan_rebeyond_memshell
{
strings:
	$s1 = "net/rebeyond/memshell/Attach.class" ascii
	$s2 = "net/rebeyond/memshell/Shell$1.class" ascii
	$s3 = "net/rebeyond/memshell/redefine/MyRequest.class" ascii
	$s4 = "net/rebeyond/memshell/Proxy.class" ascii wide
condition:
	all of them
}
rule Multios_Trojan_tsh
{
strings:
	$s1 = "<hostname|cb> get <source-file> <dest-dir>" ascii
	$s2 = "<hostname|cb> put <source-file> <dest-dir>" ascii
	$s3 = "exec bash --login" ascii
	$s4 = "tsh.handleRunShell" ascii wide
condition:
	all of them
}
rule Multios_Trojan_tshd
{
strings:
	$s1 = "connect back delay" ascii
	$s2 = "connect back host" ascii
	$s3 = "(preserved) is in daemon" ascii
	$s4 = "tshd.handlePutFile" ascii wide
condition:
	all of them
}
rule Jsp_Webshell_webshell_jsp_regeorg
{
strings:
...
rule Jsp_Webshell_webshell_jsp_http_proxy
{
strings:
...
rule Jsp_Webshell_webshell_jsp_writer_nano
{
strings:
...
rule Jsp_Webshell_webshell_jsp_generic_tiny
{
strings:
	...
	$req3 = "getRequest" fullword ascii wide
condition:
	filesize < 250 and ( $cjsp_short1 at 0 or any of ( $cjsp_long* ) or $cjsp_short2 in ( filesize-100..filesize ) or ( $cjsp_short2 and ( $cjsp_short1 in ( 0..1000 ) or $cjsp_short1 in ( filesize-1000..filesize ) ) ) ) and ( any of ( $input* ) and any of ( $req* ) ) and ( 1 of ( $payload* ) or all of ( $payload_rt* ) )
}
rule Jsp_Webshell_webshell_jsp_generic
{
strings:
...
rule Jsp_Webshell_webshell_jsp_generic_base64
{
strings:
...
rule Jsp_Webshell_webshell_jsp_generic_processbuilder
{
strings:
...
rule Jsp_Webshell_webshell_jsp_generic_reflection
{
strings:
...
rule Jsp_Webshell_webshell_jsp_generic_classloader
{
strings:
...
rule Jsp_Webshell_webshell_jsp_generic_encoded_shell
{
strings:
...
rule Jsp_Webshell_webshell_jsp_netspy
{
strings:
...
rule Jsp_Webshell_webshell_jsp_input_upload_write
{
strings:
...
rule Php_Webshell__1_c2007_php_php_c100_php
{
strings:
	$s0 = "echo \"<b>Changing file-mode (\".$d.$f.\"), \".view_perms_color($d.$f).\" (\""
	$s3 = "echo \"<td>&nbsp;<a href=\\\"\".$sql_surl.\"sql_act=query&sql_query=\".ur"
...
rule Php_Webshell__nst_php_php_img_php_php_nstview_php_php
{
strings:
	$s0 = "<tr><form method=post><td><font color=red><b>Back connect:</b></font></td><td><i"
	$s1 = "$perl_proxy_scp = \"IyEvdXNyL2Jpbi9wZXJsICANCiMhL3Vzci91c2MvcGVybC81LjAwNC9iaW4v"
...
rule Php_Webshell__network_php_php_xinfo_php_php_nfm_php_php
{
strings:
	$s0 = ".textbox { background: White; border: 1px #000000 solid; color: #000099; font-fa"
	$s2 = "<input class='inputbox' type='text' name='pass_de' size=50 onclick=this.value=''"
...
rule Php_Webshell__w_php_php_c99madshell_v2_1_php_php_wacking_php_php_SpecialShell_99_php_php
{
strings:
	$s2 = "echo \"<hr size=\\\"1\\\" noshade><b>Done!</b><br>Total time (secs.): \".$ft"
	$s3 = "$fqb_log .= \"\\r\\n------------------------------------------\\r\\nDone!\\r"
...
rule Php_Webshell__r577_php_php_SnIpEr_SA_Shell_php_r57_php_php_r57_Shell_php_php_spy_php_php_s_php_php
{
strings:
	$s2 = "'eng_text71'=>\"Second commands param is:\\r\\n- for CHOWN - name of new owner o"
	$s4 = "if(!empty($_POST['s_mask']) && !empty($_POST['m'])) { $sr = new SearchResult"
...
rule Php_Webshell__c99shell_v1_0_php_php_c99php_SsEs_php_php_ctt_sh_php_php
{
strings:
	$s0 = "\"AAAAACH5BAEAAAkALAAAAAAUABQAAAR0MMlJqyzFalqEQJuGEQSCnWg6FogpkHAMF4HAJsWh7/ze\""
	$s2 = "\"mTP/zDP//2YAAGYAM2YAZmYAmWYAzGYA/2YzAGYzM2YzZmYzmWYzzGYz/2ZmAGZmM2ZmZmZmmWZm\""
...
rule Php_Webshell__r577_php_php_spy_php_php_s_php_php
{
strings:
	$s2 = "echo $te.\"<div align=center><textarea cols=35 name=db_query>\".(!empty($_POST['"
	$s3 = "echo sr(45,\"<b>\".$lang[$language.'_text80'].$arrow.\"</b>\",\"<select name=db>"
...
rule Php_Webshell__w_php_php_c99madshell_v2_1_php_php_wacking_php_php_c99shell_v1_0_php_php_c99php_SpecialShell_99_php_php
{
strings:
	$s0 = "$sess_data[\"cut\"] = array(); c99_s"
	$s3 = "if ((!eregi(\"http://\",$uploadurl)) and (!eregi(\"https://\",$uploadurl))"
...
rule Php_Webshell__w_php_php_wacking_php_php_SpecialShell_99_php_php
{
strings:
	$s0 = "\"<td>&nbsp;<a href=\\\"\".$sql_surl.\"sql_act=query&sql_query=\".ur"
	$s2 = "c99sh_sqlquery"
...
rule Php_Webshell__w_php_php_c99madshell_v2_1_php_php_wacking_php_php_SsEs_php_php_SpecialShell_99_php_php
{
strings:
	$s0 = "else {$act = \"f\"; $d = dirname($mkfile); if (substr($d,-1) != DIRECTORY_SEPA"
	$s3 = "else {echo \"<b>File \\\"\".$sql_getfile.\"\\\":</b><br>\".nl2br(htmlspec"
...
rule Php_Webshell__r577_php_php_SnIpEr_SA_Shell_php_r57_php_php_spy_php_php_s_php_php
{
strings:
	$s0 = "echo sr(15,\"<b>\".$lang[$language.'_text"
	$s1 = ".$arrow.\"</b>\",in('text','"
...
rule Php_Webshell__r577_php_php_SnIpEr_SA_Shell_php_r57_php_php
{
strings:
	$s0 = "'ru_text9' =>'???????? ????? ? ???????? ??? ? /bin/bash'," fullword
	$s1 = "$name='ec371748dc2da624b35a4f8f685dd122'"
...
rule Php_Webshell__r577_php_php_r57_Shell_php_php_spy_php_php_s_php_php
{
strings:
	$s0 = "echo ws(2).$lb.\" <a"
	$s1 = "$sql = \"LOAD DATA INFILE \\\"\".$_POST['test3_file']"
...
rule Php_Webshell__wacking_php_php_1_SpecialShell_99_php_php_c100_php
{
strings:
	$s0 = "if(eregi(\"./shbd $por\",$scan))"
	$s1 = "$_POST['backconnectip']"
...
rule Php_Webshell__r577_php_php_r57_php_php_r57_Shell_php_php_spy_php_php_s_php_php
{
strings:
	$s1 = "if(rmdir($_POST['mk_name']))"
	$s2 = "$r .= '<tr><td>'.ws(3).'<font face=Verdana size=-2><b>'.$key.'</b></font></td>"
...
rule Php_Webshell__w_php_php_wacking_php_php_SsEs_php_php_SpecialShell_99_php_php
{
strings:
	$s0 = "\"ext_avi\"=>array(\"ext_avi\",\"ext_mov\",\"ext_mvi"
	$s1 = "echo \"<b>Execute file:</b><form action=\\\"\".$surl.\"\\\" method=POST><inpu"
...
rule Php_Webshell__w_php_php_c99madshell_v2_1_php_php_wacking_php_php
{
strings:
	$s0 = "<b>Dumped! Dump has been writed to "
	$s1 = "if ((!empty($donated_html)) and (in_array($act,$donated_act))) {echo \"<TABLE st"
...
rule Php_Webshell__w_php_php_c99madshell_v2_1_php_php_wacking_php_php_c99shell_v1_0_php_php_c99php
{
strings:
	$s0 = "@ini_set(\"highlight" fullword
	$s1 = "echo \"<b>Result of execution this PHP-code</b>:<br>\";" fullword
...
rule Php_Webshell__GFS_web_shell_ver_3_1_7___PRiV8_php_nshell_php_php_gfs_sh_php_php
{
strings:
	$s2 = "echo $uname.\"</font><br><b>\";" fullword
	$s3 = "while(!feof($f)) { $res.=fread($f,1024); }" fullword
...
rule Php_Webshell__w_php_php_c99madshell_v2_1_php_php_wacking_php_php_c99shell_v1_0_php_php_SpecialShell_99_php_php
{
strings:
	$s0 = "c99ftpbrutecheck"
	$s1 = "$ftpquick_t = round(getmicrotime()-$ftpquick_st,4);" fullword
...
rule Php_Webshell__w_php_php_wacking_php_php_c99shell_v1_0_php_php_c99php_SpecialShell_99_php_php
{
strings:
	$s0 = "$sqlquicklaunch[] = array(\""
	$s1 = "else {echo \"<center><b>File does not exists (\".htmlspecialchars($d.$f).\")!<"
...
rule Php_Webshell__antichat_php_php_Fatalshell_php_php_a_gedit_php_php
{
strings:
	$s0 = "if(@$_POST['save'])writef($file,$_POST['data']);" fullword
	$s1 = "if($action==\"phpeval\"){" fullword
...
rule Php_Webshell__c99shell_v1_0_php_php_c99php_SsEs_php_php
{
strings:
	$s3 = "if (!empty($delerr)) {echo \"<b>Deleting with errors:</b><br>\".$delerr;}" fullword
condition:
...
rule Php_Webshell__Crystal_php_nshell_php_php_load_shell_php_php
{
strings:
	$s0 = "if ($filename != \".\" and $filename != \"..\"){" fullword
	$s1 = "$dires = $dires . $directory;" fullword
...
rule Php_Webshell__nst_php_php_cybershell_php_php_img_php_php_nstview_php_php
{
strings:
	$s0 = "@$rto=$_POST['rto'];" fullword
	$s2 = "SCROLLBAR-TRACK-COLOR: #91AAFF" fullword
...
rule Php_Webshell__w_php_php_c99madshell_v2_1_php_php_wacking_php_php_dC3_Security_Crew_Shell_PRiV_php_SpecialShell_99_php_php
{
strings:
	$s0 = " if ($mode & 0x200) {$world[\"execute\"] = ($world[\"execute\"] == \"x\")?\"t\":"
	$s1 = " $group[\"execute\"] = ($mode & 00010)?\"x\":\"-\";" fullword
...
rule Php_Webshell__c99shell_v1_0_php_php_c99php_1_c2007_php_php_c100_php
{
strings:
	$s0 = "$result = mysql_query(\"SHOW PROCESSLIST\", $sql_sock); " fullword
condition:
...
rule Php_Webshell__w_php_php_c99madshell_v2_1_php_php_wacking_php_php_1_SpecialShell_99_php_php
{
strings:
	$s0 = "if ($total === FALSE) {$total = 0;}" fullword
	$s1 = "$free_percent = round(100/($total/$free),2);" fullword
...
rule Php_Webshell__r577_php_php_r57_php_php_spy_php_php_s_php_php
{
strings:
	$s0 = "$res = mssql_query(\"select * from r57_temp_table\",$db);" fullword
	$s2 = "'eng_text30'=>'Cat file'," fullword
...
rule Php_Webshell__nixrem_php_php_c99shell_v1_0_php_php_c99php_NIX_REMOTE_WEB_SHELL_v_0_5_alpha_Lite_Public_Version_php
{
strings:
	$s0 = "$num = $nixpasswd + $nixpwdperpage;" fullword
	$s1 = "$ret = posix_kill($pid,$sig);" fullword
...
rule Php_Webshell__root_040_zip_Folder_deploy
{
strings:
	$s5 = "halon synscan 127.0.0.1 1-65536"
	$s8 = "Obviously you replace the ip address with that of the target."
...
rule Unix_Malware_Bash_e427876d
{
strings:
...
rule Unix_Malware_Fontonlake_fe916a45
{
strings:
...
rule Unix_Malware_Generic_babf9101
{
strings:
...
rule Unix_Malware_Generic_5776ae49
{
strings:
...
rule Unix_Malware_Python_00606bac
{
strings:
...
rule Unix_Malware_Tinyshell_67ee6fae
{
strings:
...
rule Unix_Coinminer_Attribute_3683d149
{
strings:
...
rule Unix_Coinminer_Bscope_348b7fa0
{
strings:
...
rule Unix_Coinminer_Bulz_2aa8fbb5
{
strings:
...
rule Unix_Coinminer_Bulz_0998f811
{
strings:
...
rule Unix_Coinminer_Camelot_9ac1654b
{
strings:
...
rule Unix_Coinminer_Camelot_dd167aa0
{
strings:
...
rule Unix_Coinminer_Camelot_b25398dd
{
strings:
...
rule Unix_Coinminer_Camelot_6a279f19
{
strings:
...
rule Unix_Coinminer_Camelot_4e7945a4
{
strings:
...
rule Unix_Coinminer_Camelot_29c1c386
{
strings:
...
rule Unix_Coinminer_Camelot_25b63f54
{
strings:
...
rule Unix_Coinminer_Camelot_73e2373e
{
strings:
...
rule Unix_Coinminer_Camelot_b8552fff
{
strings:
...
rule Unix_Coinminer_Camelot_83550472
{
strings:
...
rule Unix_Coinminer_Camelot_8799d8d6
{
strings:
...
rule Unix_Coinminer_Camelot_0f7c5375
{
strings:
...
rule Unix_Coinminer_Camelot_87639dbd
{
strings:
...
rule Unix_Coinminer_Camelot_cdd631c1
{
strings:
...
rule Unix_Coinminer_Camelot_209b02dd
{
strings:
...
rule Unix_Coinminer_Casdet_5d0d33be
{
strings:
...
rule Unix_Coinminer_Ccminer_18fc60e5
{
strings:
...
rule Unix_Coinminer_Ccminer_3c593bc3
{
strings:
...
rule Unix_Coinminer_Flystudio_579a3a4d
{
strings:
...
rule Unix_Coinminer_Generic_467c4d46
{
strings:
	$a1 = { CF 99 67 D8 37 AA 24 80 F2 F3 47 6A A5 5E 88 50 F1 28 61 18 }
	$a2 = { 43 08 49 89 46 08 48 8B 43 10 49 89 46 10 48 85 C0 74 8A F0 83 40 }
	$a3 = { 74 4F 48 8B 75 00 48 8B 4D 08 4C 89 F7 48 8B 55 10 48 8B 45 18 48 89 74 }
	$a4 = { F0 4C 01 F0 49 8B 75 08 48 01 C3 49 39 F4 74 29 48 89 DA 4C }
	$a5 = { 72 59 D4 B5 63 E2 4D B6 08 EF E8 0A 3A B1 AD 1B 61 6E 7C 65 D1 }
	$a6 = { 49 10 58 00 10 D4 34 80 08 30 01 20 02 00 B1 00 83 49 23 16 54 }
	$a7 = { C1 48 29 D0 48 01 C0 4D 8B 39 48 29 C1 49 29 F8 48 8D 04 C9 4D 8D }
	$a8 = { 0F 94 C0 EB 05 B8 01 00 00 00 44 21 E8 48 8B 4C 24 08 64 48 33 0C 25 28 00 }
	$a9 = { FD 48 8D 5D 01 4C 8D 14 1B 48 C1 E3 05 4C 01 EB 4D 8D 7A FF F2 0F }
	$a10 = { 2C 1C 77 16 48 8B 44 24 08 64 48 33 04 25 28 00 00 00 75 24 48 83 }
	$a11 = { 46 08 48 89 45 08 48 8B 46 10 48 85 C0 48 89 45 10 74 BC F0 FF }
	$a12 = { CB 49 C1 E3 04 49 01 FB 41 8B 13 39 D1 7F 3F 7C 06 4D 3B 43 08 }
	$a13 = { 8B 04 25 28 00 00 00 48 89 44 24 58 31 C0 49 83 FF 3F 48 89 74 }
	$a14 = { 45 A8 8B 00 89 C2 48 8B 45 C8 48 01 C2 8B 45 90 48 39 C2 7E 08 8B }
	$a15 = { C4 10 5B C3 E8 35 A7 F6 FF 0F 1F 44 00 00 53 48 }
	$a16 = { 79 73 00 6E 6F 5F 6D 6C 63 6B 00 77 61 72 6E 00 6E 65 76 65 }
	$a17 = { 0F B6 4F 3D 0B 5C 24 F4 41 C1 EB 10 44 0B 5C 24 }
	$a18 = { 70 D2 AA C5 F9 EF D2 C5 F1 EF CB C5 E1 73 FB 04 C4 E3 79 DF }
	$a19 = { 6F 67 31 70 00 6C 6F 67 32 66 00 6C 6C 72 6F 75 6E 64 00 73 71 }
	$a20 = { 8C 24 98 00 00 00 31 C9 80 7A 4A 00 48 89 74 24 18 48 89 54 }
	$a21 = { 4B 04 31 C0 41 8B 14 07 89 14 01 48 83 C0 04 48 83 F8 14 75 EF 4C 8D 74 }
	$a22 = { 00 70 6F 77 00 6C 6F 67 31 70 00 6C 6F 67 32 66 00 63 65 69 6C 00 }
	$a23 = { 0C 14 89 0C 10 48 83 C2 04 48 83 FA 20 75 EF 48 8D 8C 24 F0 00 }
	$a24 = { 47 20 49 8D 77 20 4C 89 74 24 10 4C 89 6C 24 18 4C 89 64 24 20 }
	$a25 = { 47 08 49 3B 47 10 74 3C 48 85 C0 74 16 48 8B 13 48 89 10 48 8B }
	$a26 = { 04 62 20 0A 10 02 0A 14 60 29 00 02 0C 24 14 60 7D 44 01 70 01 }
	$a27 = { D9 4D 01 CA 4C 89 74 24 D0 4C 8B 74 24 E8 4D 31 D4 49 C1 C4 20 48 C1 }
	$a28 = { EE 6A 00 41 B9 01 00 00 00 48 8D 4A 13 4C 89 E7 88 85 40 FF }
	$a29 = { 2C 24 74 67 48 89 5C 24 18 4C 89 6C 24 20 4C 89 FB 4D 89 E5 4C 8B }
	$a30 = { 07 41 89 34 06 48 83 C0 04 48 83 F8 20 75 EF 8B 42 D4 66 0F }
	$a31 = { 89 D8 48 83 C4 20 5B C3 0F 1F 00 BF ?? ?? 40 00 }
	$a32 = { 41 56 45 31 F6 41 55 49 89 F5 41 54 44 8D 67 01 55 4D 63 E4 53 49 C1 }
	$a33 = { 30 42 30 42 00 22 22 03 5C DA 10 00 C0 00 60 43 9C 64 48 00 00 00 }
	$a34 = { 24 FC 00 00 00 8B 84 24 C0 00 00 00 0F 29 84 24 80 00 00 00 0F 11 94 24 C4 00 }
	$a35 = { EA 48 89 DE 48 8D 7C 24 08 FF 53 18 48 8B 44 24 08 48 83 78 }
	$a36 = { 0F 6F D0 66 44 0F 6F C1 66 0F 69 E2 66 44 0F 61 D2 66 44 0F }
	$a37 = { 71 F2 08 66 0F EF C1 66 0F EF D3 66 0F 7F 44 24 60 66 0F 7F 54 }
	$a38 = { 48 C1 E6 05 48 01 C6 48 39 F1 74 05 49 89 74 24 08 44 89 E9 48 C1 }
	$a39 = { 03 51 8A 92 FF F3 20 01 DE 63 AF 8B 54 73 0A 65 83 64 88 60 }
	$a40 = { 71 18 4C 89 FF FF D0 48 8B 84 24 A0 00 00 00 48 89 43 60 48 8B 84 24 98 00 }
	$a41 = { 73 F2 06 C5 F9 EB C2 C4 E3 79 16 E0 02 C4 E3 79 16 E2 03 C5 F9 70 }
	$a42 = { 49 8B 77 08 48 21 DE 4C 39 EE 75 CE 66 41 83 7F 1E 04 4C 89 F5 }
	$a43 = { 54 24 40 48 8D 94 24 C0 00 00 00 F3 41 0F 6F 01 48 89 7C 24 50 48 89 74 }
	$a44 = { 61 6C 73 65 20 70 6F 73 69 74 69 76 65 29 1B 5B 30 6D 00 44 45 }
condition:
	1 of them
}
rule Unix_Coinminer_Flystudio_0a370634
{
strings:
...
rule Unix_Coinminer_Loudminer_581f57a9
{
strings:
...
rule Unix_Coinminer_Loudminer_f2298a50
{
strings:
...
rule Unix_Coinminer_Loudminer_851fc7aa
{
strings:
...
rule Unix_Coinminer_Malxmr_d13544d7
{
strings:
...
rule Unix_Coinminer_Malxmr_ad09e090
{
strings:
...
rule Unix_Coinminer_Malxmr_12299814
{
strings:
...
rule Unix_Coinminer_Malxmr_a47b77e4
{
strings:
...
rule Unix_Coinminer_Malxmr_21d0550b
{
strings:
...
rule Unix_Coinminer_Malxmr_c8adb449
{
strings:
...
rule Unix_Coinminer_Malxmr_bcab1e8f
{
strings:
...
rule Unix_Coinminer_Malxmr_6671f33a
{
strings:
...
rule Unix_Coinminer_Malxmr_74418ec5
{
strings:
...
rule Unix_Coinminer_Malxmr_979160f6
{
strings:
...
rule Unix_Coinminer_Malxmr_fe7139e5
{
strings:
...
rule Unix_Coinminer_Malxmr_f35a670c
{
strings:
...
rule Unix_Coinminer_Malxmr_70e5946e
{
strings:
...
rule Unix_Coinminer_Malxmr_033f06dd
{
strings:
...
rule Unix_Coinminer_Malxmr_ce0c185f
{
strings:
...
rule Unix_Coinminer_Malxmr_da08e491
{
strings:
...
rule Unix_Coinminer_Miancha_646803ef
{
strings:
...
rule Unix_Coinminer_Minertr_9901e275
{
strings:
...
rule Unix_Coinminer_Pgminer_ccf88a37
{
strings:
...
rule Unix_Coinminer_Pgminer_5fb2efd5
{
strings:
...
rule Unix_Coinminer_Presenoker_3bb5533d
{
strings:
...
rule Unix_Coinminer_Roboto_0b6807f8
{
strings:
...
rule Unix_Coinminer_Roboto_1f1cfe9a
{
strings:
...
rule Unix_Coinminer_Stak_05088561
{
strings:
...
rule Unix_Coinminer_Stak_ae8b98a9
{
strings:
...
rule Unix_Coinminer_Stak_d707fd3a
{
strings:
...
rule Unix_Coinminer_Stak_52dc7af3
{
strings:
...
rule Unix_Coinminer_Stak_bb3153ac
{
strings:
...
rule Unix_Coinminer_Ursu_3c05f8ab
{
strings:
...
rule Unix_Coinminer_Uwamson_c42fd06d
{
strings:
...
rule Unix_Coinminer_Uwamson_d08b1d2e
{
strings:
...
rule Unix_Coinminer_Uwamson_0797de34
{
strings:
...
rule Unix_Coinminer_Uwamson_41e36585
{
strings:
...
rule Unix_Coinminer_Xmrig_57c0c6d7
{
strings:
...
rule Unix_Coinminer_Xmrig_7e42bf80
{
strings:
...
rule Unix_Coinminer_Xmrig_271121fb
{
strings:
...
rule Unix_Coinminer_Xmrig_e7e64fb7
{
strings:
...
rule Unix_Coinminer_Xmrig_79b42b21
{
strings:
...
rule Unix_Coinminer_Xmrig_77fbc695
{
strings:
...
rule Unix_Coinminer_Xmrig_403b0a12
{
strings:
...
rule Unix_Coinminer_Xmrig_bffa106b
{
strings:
...
rule Unix_Coinminer_Xmrig_73faf972
{
strings:
...
rule Unix_Coinminer_Xmrig_af809eea
{
strings:
...
rule Unix_Coinminer_Xmrig_9f6ac00f
{
strings:
...
rule Unix_Coinminer_Xmrig_dbcc9d87
{
strings:
...
rule Unix_Coinminer_Xmrminer_70c153b5
{
strings:
...
rule Unix_Coinminer_Xmrminer_98b00f9c
{
strings:
...
rule Unix_Coinminer_Xmrminer_2b250178
{
strings:
...
rule Unix_Coinminer_Xmrminer_67bf4b54
{
strings:
...
rule Unix_Coinminer_Xmrminer_504b42ca
{
strings:
...
rule Unix_Coinminer_Xmrminer_d1bb752f
{
strings:
...
rule Unix_Coinminer_Xmrminer_d625fcd2
{
strings:
...
rule Unix_Coinminer_Xmrminer_02d19c01
{
strings:
...
rule Unix_Coinminer_Xmrminer_2dd045fc
{
strings:
...
rule Unix_Coinminer_Xmrminer_d1a814b0
{
strings:
...
rule Unix_Coinminer_Xmrminer_c6218e30
{
strings:
...
rule Unix_Coinminer_Xmrminer_b17a7888
{
strings:
...
rule Unix_Coinminer_Xpaj_fdbd614e
{
strings:
...
rule Unix_Coinminer_Zexaf_b90e7683
{
strings:
...
rule Unix_Dropper_Generic_0bd15ae0
{
strings:
...
rule Unix_Exploit_Abrox_5641ba81
{
strings:
...
rule Unix_Exploit_Alie_e69de1ee
{
strings:
...
rule Unix_Exploit_CVE_2009_1897_6cf0a073
{
strings:
...
rule Unix_Exploit_CVE_2009_2698_12374e97
{
strings:
...
rule Unix_Exploit_CVE_2009_2698_cc04dddd
{
strings:
...
rule Unix_Exploit_CVE_2009_2908_406c2fef
{
strings:
...
rule Unix_Exploit_CVE_2010_3301_79d52efd
{
strings:
...
rule Unix_Exploit_CVE_2010_3301_d0eb0924
{
strings:
...
rule Unix_Exploit_CVE_2010_3301_a5828970
{
strings:
...
rule Unix_Exploit_CVE_2012_0056_06b2dff5
{
strings:
...
rule Unix_Exploit_CVE_2012_0056_b39839f4
{
strings:
...
rule Unix_Exploit_CVE_2012_0056_a1e53450
{
strings:
...
rule Unix_Exploit_CVE_2014_3153_1c1e02ad
{
strings:
...
rule Unix_Exploit_CVE_2016_4557_b7e15f5e
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_364f3b7b
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_3a2ed31b
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_7448814c
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_2fa988e3
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_ea8801ac
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_b2ebdebd
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_9190d516
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_3b460716
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_ccfd7518
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_d41c2c63
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_ffa7f059
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_fb24c7e4
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_b45098df
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_9c67a994
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_ab87c1ed
{
strings:
...
rule Unix_Exploit_CVE_2016_5195_f1c0482a
{
strings:
...
rule Unix_Exploit_CVE_2017_100011_21025f50
{
strings:
...
rule Unix_Exploit_CVE_2017_16995_0c81a317
{
strings:
...
rule Unix_Exploit_CVE_2017_16995_82816caa
{
strings:
...
rule Unix_Exploit_CVE_2021_3156_f3fb10cd
{
strings:
...
rule Unix_Exploit_CVE_2021_3156_7f5672d0
{
strings:
...
rule Unix_Exploit_CVE_2021_3490_d369d615
{
strings:
...
rule Unix_Exploit_CVE_2021_4034_1c8f235d
{
strings:
	$s1 = "PATH=GCONV_PATH="
	$s2 = "pkexec"
...
rule Unix_Exploit_CVE_2022_0847_e831c285
{
strings:
	$pp = "prepare_pipe"
	$s1 = "splice failed"
...
rule Unix_Exploit_Log4j_7fc4d480
{
strings:
...
rule Unix_Tool_Fontonlake_68ad8568
{
strings:
	$s1 = "run_in_bash"
	$s2 = "run_in_ss"
...
rule Unix_Tool_Wipelog_daea1aa4
{
strings:
...
rule Unix_Rootkit_Fontonlake_8fa41f5e
{
strings:
...
rule Unix_Trojan_BPFDoor_59e029c3
{
strings:
...
rule Unix_Trojan_BPFDoor_0f768f60
{
strings:
	$a1 = "hald-addon-acpi: listening on acpi kernel interface /proc/acpi/event" ascii fullword
...
rule Unix_Trojan_BPFDoor_8453771b
{
strings:
...
rule Unix_Trojan_BPFDoor_1a7d804b
{
strings:
...
rule Unix_Trojan_BPFDoor_e14b0b79
{
strings:
...
rule Multios_Trojan_Bishopsliver_42298c4a
{
strings:
	$a1 = ").RequestResend"
	$a2 = ").GetPrivInfo"
